...
const OUTPUT_PATH =
    path.join(__dirname, "../data/pokedex.updated.json");
...
async function buildPokemon(id) {
    ...
    if (currentNode?.evolves_to?.length) {
        evolution.next =
            currentNode.evolves_to.map(child => {
                const nextId =
                    child.species.url
                        .split("/")
                        .filter(Boolean)
...
async function main() {
    ...
    console.log(
        `Pokédex atualizada em: ${OUTPUT_PATH}`
    );

    console.log(
        `Novos Pokémon adicionados: ${newPokemons.length}`
    );
}
